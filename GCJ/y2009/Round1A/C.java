 ...
 * Type: DP/Probability
 * Solution: 
 * Make a one dimensional DP table. Dp[i] = expected # of packs needed to
get one card of each type given you have i different types of cards
already.

Now to compute dp[i]. You can use combinatorics to find the
probability the next pack of cards will give you 0,1,2,... number of
new cards.

dp[i] = (prob of 0 more)(1+dp[i]) + (prob of 1 more)(1+dp[i+1])+...
solve for dp[i]
 */
